...
async function testPerformance() {
  ...
  const testText =
    "Malgré que j'aie des difficultés, je vais pallier à ce problème au jour d'aujourd'hui. Au final, il faut que je sois plus attentif.";
  const iterations = 5;
...
async function runAllTests() {
  ...
  try {
    ...
    console.log('\n📋 Résumé des tests:');
    console.log(
      `   Tests de grammaire: ${grammarResults.passed}/${grammarResults.total} (${grammarResults.successRate}%)`
    );
