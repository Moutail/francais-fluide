...
export const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({
  ...
  const [optimizationResults, setOptimizationResults] = useState(performanceOptimizer.getOptimizationResults());
  ...
  return (
    <div className={`performance-dashboard ${className}`}>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-3">Composants Lazy</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-gray-50 rounded-lg p-3">
              <div className="text-sm text-gray-600">Total</div>
              <div className="text-xl font-bold text-gray-900">{lazyLoaderMetrics.totalComponents}</div>
            </div>
            <div className="bg-green-50 rounded-lg p-3">
              <div className="text-sm text-green-600">Chargés</div>
              <div className="text-xl font-bold text-green-700">{lazyLoaderMetrics.loadedComponents}</div>
            </div>
            <div className="bg-yellow-50 rounded-lg p-3">
              <div className="text-sm text-yellow-600">En cours</div>
              <div className="text-xl font-bold text-yellow-700">{lazyLoaderMetrics.loadingComponents}</div>
            </div>
            <div className="bg-red-50 rounded-lg p-3">
              <div className="text-sm text-red-600">Erreurs</div>
              <div className="text-xl font-bold text-red-700">{lazyLoaderMetrics.errorComponents}</div>
            </div>
          </div>
        </div>
        ...
        {optimizationResults.length > 0 && (
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">Optimisations Récentes</h3>
              ...
              {optimizationResults.slice(-3).map((result, index) => (
                ...
                >
                  ...
                  <div className={`text-sm font-medium ${
                    ...
                    {result.improvement > 0 ? '+' : ''}{result.improvement.toFixed(1)} pts
                  </div>
                </motion.div>
              ))}
            </div>
