...
import { AlertTriangle, TrendingDown, Clock, Target, BarChart3, Brain, Zap } from 'lucide-react';
...
export default function ErrorAnalytics() {
  ...
  const getErrorTypeLabel = (type: string) => {
    switch (type) {
      case 'answer_selected':
        return 'Réponse sélectionnée';
      case 'answer_changed':
        return 'Changement de réponse';
      case 'question_skipped':
        return 'Question ignorée';
      case 'question_completed':
        return 'Question complétée';
      default:
        return type;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-xl">
        ...
        {sortedPatterns.length === 0 ? (
          <p className="py-8 text-center text-gray-500">Aucun pattern d'erreur détecté</p>
        ) : (
          <div className="space-y-3">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-xl">
        ...
        {Object.keys(analytics.averageResponseTimes).length === 0 ? (
          <p className="py-8 text-center text-gray-500">Aucune donnée de temps de réponse</p>
        ) : (
          <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">{stats.count} questions analysées</div>
                  </div>
                </div>
                <div className="text-right">
      ...
      {/* Recommandations IA */}
      <div className="rounded-2xl border border-purple-200 bg-gradient-to-r from-purple-50 to-blue-50 p-6 shadow-xl">
        <h4 className="mb-4 flex items-center gap-2 text-lg font-semibold text-gray-900">
          ...
          {sortedPatterns.slice(0, 3).map((pattern, index) => (
            <div key={index} className="rounded-lg border border-purple-200 bg-white p-3">
              <div className="text-sm text-gray-700">
                <strong>Pattern détecté:</strong> {getErrorTypeLabel(pattern.type)} ({pattern.count}{' '}
                fois)
              </div>
              <div className="mt-1 text-xs text-gray-600">
                L'IA peut adapter la difficulté ou proposer des exercices ciblés pour ce type
                d'erreur.
              </div>
            </div>
          ))}
