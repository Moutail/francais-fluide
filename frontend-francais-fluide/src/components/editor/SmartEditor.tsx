...
export const SmartEditor: React.FC<SmartEditorProps> = ({
  ...
  // Analyse du texte avec debounce
  useEffect(() => {
    ...
    const analyzeText = async () => {
      ...
      try {
        ...
        setErrors(result.errors.map((error, index) => ({
    ...
    analyzeText();
  }, [debouncedText, realTimeCorrection]);

  // Gestion du texte avec highlighting
  ...
  // Rendu des erreurs avec overlays
      ...
      {isAnalyzing ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={cn(
            ...
            metrics.accuracyRate >= 90 ? "bg-green-100" :
            metrics.accuracyRate >= 70 ? "bg-yellow-100" : "bg-red-100"
          )}>
            <span className="text-xs font-semibold">
              {metrics.accuracyRate}% précision
            </span>
          </div>
        </div>
      )}
        ...
        >
          <div className="flex items-center justify-between text-xs text-gray-600">
            <div className="flex items-center gap-4">
              <span>{metrics.wordsWritten} mots</span>
              {errors.length > 0 && (
            ...
            <div className="flex items-center gap-2">
              ...
              <span>Mode {mode === 'practice' ? 'Entraînement' : mode === 'exam' ? 'Examen' : 'Créatif'}</span>
            </div>
          </div>
        </motion.div>
      </div>
