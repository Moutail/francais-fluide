      ...
      <div className="flex items-center gap-3">
        {perfectStreak > 2 && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="flex items-center gap-1 rounded-full bg-green-100 px-2 py-1"
          >
        ...
        >
          <span className="text-xs font-semibold">{accuracyRate}% précision</span>
        </div>
      </div>
    )}
  </motion.div>
  ...
  <AnimatePresence>
    {showSuggestions && selectedError && (
      <motion.div
        ...
        exit={{ opacity: 0, y: -10 }}
        className="absolute z-50 max-w-sm rounded-lg border border-gray-200 bg-white p-4 shadow-2xl"
        style={{
      ...
      >
        <div className="mb-3 flex items-start gap-2">
          ...
          />
          <div>
            <p className="text-sm font-medium text-gray-900">{selectedError.message}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          {selectedError.suggestions.map((suggestion, index) => (
            <motion.button
              ...
              className="w-full rounded-md bg-gradient-to-r from-blue-50 to-indigo-50 px-3 py-2 text-left transition-colors hover:from-blue-100 hover:to-indigo-100"
            >
  ...
  ({
    ...
    const handleTextChange = useCallback(
      (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        if (selectedError) {
          const errorStillExists = errors.some(
            err => err.start <= e.target.selectionStart && err.end >= e.target.selectionStart
          );
          if (!errorStillExists) {
      ...
      return errors.map(error => {
        ...
        return (
          <motion.span
            key={error.id}
            className={cn(
              'absolute cursor-pointer border-b-2 transition-all',
              color,
      ...
      <div className={cn('relative', className)}>
        <div className="relative overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="absolute right-2 top-2 flex items-center gap-2">
            <LanguageToolStatus isAvailable={isAvailable} isChecking={isChecking} />
          ...
          <div className="relative">
            <TextareaAutosize
              ...
              className="w-full resize-none p-4 pt-12 font-sans leading-relaxed text-gray-900 outline-none"
              minRows={8}
              maxRows={20}
              style={{
            ...
            <div
              ref={overlayRef}
              className="pointer-events-none absolute inset-0 p-4 pt-12"
              style={{
                font: 'inherit',
                whiteSpace: 'pre-wrap',
                wordWrap: 'break-word',
          ...
          {/* Barre de statut */}
          <motion.div
            className="border-t border-gray-200 bg-gradient-to-r from-gray-50 to-gray-100 px-4 py-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
              ...
              <div className="flex items-center gap-4">
                ...
                {statusBarContent.errorCount > 0 && (
                  <span className="font-medium text-orange-600">
                    {statusBarContent.errorCount} suggestion
                    {statusBarContent.errorCount > 1 ? 's' : ''}
                  </span>
                )}
              </div>
