    ...
    questions: [
      ...
      },
      {
        ...
        options: [
          ...
          'Ils regagnent leurs nids',
          'Ils cherchent de la nourriture',
          'Ils volent en formation',
        ],
...

  {
    ...
    questions: [
      ...
      },
      {
        ...
        explanation:
          '"Au jour d\'aujourd\'hui" est un pléonasme. Utilisez simplement "aujourd\'hui".',
      },
    ],
    scoring: {
      maxPoints: 250,
...
export class ExerciseBank {
  ...
  getSeries(count: number, difficulty?: Difficulty, type?: ExerciseType): Exercise[] {
    ...
    const available = this.exercises.filter(
      ex => (!difficulty || ex.difficulty === difficulty) && (!type || ex.type === type)
    );
