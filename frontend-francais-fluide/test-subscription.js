  ...
  for (const feature of features) {
    ...
    try {
      ...
      if (hasAccess === hasAccessResponse) {
        ...
        console.log(`  ❌ ${feature.name}: Accès incorrect (attendu: ${hasAccess}, reçu: ${hasAccessResponse})`);
      }
    } catch (error) {
  ...
  try {
    ...
    if (exercisesData.success && exercisesData.data.length > 0) {
      ...
      if (submissionData.success) {
        ...
        console.log(`  Réponses correctes: ${submissionData.data.correctAnswers}/${submissionData.data.totalQuestions}`);
      } else {
