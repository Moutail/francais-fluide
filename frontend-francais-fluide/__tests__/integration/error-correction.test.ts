...
describe('Scénarios de correction d\'erreurs - Intégration', () => {
  ...
  describe('Gestion des erreurs complexes', () => {
    ...
    it('gère les erreurs avec des métadonnées complexes', async () => {
      const user = userEvent.setup();
        ...
        errors: [
          {
            offset: 0,
            length: 10,
            message: 'Erreur complexe avec contexte',
      ...
      const textarea = screen.getByRole('textbox');
      await user.type(textarea, 'phrase avant mot_incorrect phrase après');
      ...
      await waitFor(() => {
        expect(screen.getByText('Erreur complexe avec contexte')).toBeInTheDocument();
